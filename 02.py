...
class LuckAward:
    ...
    def __init__(self):
        ...
        self.root.title('俄罗斯大转盘')
        self.root.minsize(400,400)
        self.root['bg'] = 'cyan'
        self.coin = 0

        self.lable = tkinter.Label(self.root, text='游戏币：{}'.format(0))
        self.lable.pack()
        #设置布局
        self.pack_labels()
        # 将所有标签组件组成一个列表
    ...
    def pack_labels(self):
        ...
        self.label11 = tkinter.Label(self.root,text='苹果X', bg='white')
        self.label11.place(relx=0.08, rely=0.08, relwidth=0.15, relheight=0.15)

        self.label12 = tkinter.Label(self.root,text='Ipad', bg='white')
        self.label12.place(relx=0.31, rely=0.08, relwidth=0.15, relheight=0.15)

        self.label13 = tkinter.Label(self.root,text='Iwatch', bg='white')
        self.label13.place(relx=0.54, rely=0.08, relwidth=0.15, relheight=0.15)

        self.label14 = tkinter.Label(self.root,text='Mac笔记本', bg='white')
        self.label14.place(relx=0.77, rely=0.08, relwidth=0.15, relheight=0.15)

        self.label21 = tkinter.Label(self.root,text='500积分', bg='white')
        self.label21.place(relx=0.08, rely=0.31, relwidth=0.15, relheight=0.15)

        self.label24 = tkinter.Label(self.root,text='再来一次', bg='white')
        self.label24.place(relx=0.77, rely=0.31, relwidth=0.15, relheight=0.15)

        self.label31 = tkinter.Label(self.root,text='500元话费', bg='white')
        self.label31.place(relx=0.08, rely=0.54, relwidth=0.15, relheight=0.15)

        self.label34 = tkinter.Label(self.root,text='QQ黑钻', bg='white')
        self.label34.place(relx=0.77, rely=0.54, relwidth=0.15, relheight=0.15)

        self.label41 = tkinter.Label(self.root,text='Kindel', bg='white')
        self.label41.place(relx=0.08, rely=0.77, relwidth=0.15, relheight=0.15)

        self.label42 = tkinter.Label(self.root,text='谢谢惠顾', bg='white')
        self.label42.place(relx=0.31, rely=0.77, relwidth=0.15, relheight=0.15)

        self.label43 = tkinter.Label(self.root,text='再来一次', bg='white')
        self.label43.place(relx=0.54, rely=0.77, relwidth=0.15, relheight=0.15)

        self.label44 = tkinter.Label(self.root,text='QQ黄钻', bg='white')
        self.label44.place(relx=0.77, rely=0.77, relwidth=0.15, relheight=0.15)

        #摆放开始/停止按钮组件
        self.btn1 = tkinter.Button(self.root,text='开始', bg='yellow', font=('黑体', 20), command=self.new_threading)
        self.btn1.place(relx=0.26, rely=0.40, relwidth=0.2, relheight=0.15)

        self.btn2 = tkinter.Button(self.root,text='停止', bg='red', font=('黑体', 20), command=self.stop)
        self.btn2.place(relx=0.54, rely=0.40, relwidth=0.2, relheight=0.15)

        self.btn3 = tkinter.Button(self.root, text='coin', bg='blue', font=('黑体', 20), command=self.add)
        self.btn3.place(relx=0.40, rely=0.60, relwidth=0.2, relheight=0.15)
    ...
    def new_threading(self):
        ...
        if self.isrotate == False:
            # 弹出‘未开始点击结束’的提示信息
            result = tkinter.messagebox.askokcancel(title='提示框', message='十块钱玩一把！！')
        # 开始游戏
        if self.coin > 0:
            # 建立线程
            self.coin -= 10
            self.lable["text"] = '游戏币：{}'.format(self.coin)
            self.stopsign = False
            # 建立新线程
            ...
            tkinter.messagebox.showerror('错误', '没有币了，请投币')
    ...
    def stop(self):
        ...
        if self.isrotate == False:
            #弹出‘未开始点击结束’的提示信息
            result = tkinter.messagebox.askokcancel(title = '提示框',message = '请投币！！')
            if result == True:
                self.new_threading()
            else:
